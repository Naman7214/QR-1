...
def check_existing_records(subject_name, time_slot, date):
    teacher_id = session.get('teacher_id')  # Use get() to avoid potential KeyError
...
# def process_timetable_data(timetable_data, elective_data):
#     processed_timetable = []
...
#         if is_elective:
#             # Fetch elective details from the elective_data table
#             elective_subject = subject_name
#             for elective_row in elective_data:
#                 if elective_row[0] == elective_subject:
#                     subject_name = elective_row[1]
#                     faculty = elective_row[2]
#                     break

#         processed_timetable.append({
#             'day': day,
#             'time_slot': time_slot,
...
def generate_qr_code_from_input(subject_name, time_slot, date, year):
    ...
    if year == 'SE':
        ...
        for student in students:
            db.execute('INSERT INTO Temp_attendance (rollno, stdname, subject, date, time, attendance, teacher_id, year) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                        (student['roll_no'], student['name'], subject_name, date, time_slot, 0, session['teacher_id'], year))
        ...
        if subject_name == "CS" or subject_name == "CC":
            ...
            if students is not None:
                ...
                for student in students:
                    db.execute('INSERT INTO Temp_attendance (rollno, stdname, subject, date, time, attendance, teacher_id, year) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                            (student['roll_no'], student['name'], subject_name, date, time_slot, 0, teacher_id, year))
                db.commit()
...
def input():
    ...
    if request.method == 'POST':
        ...
        if check_existing_records(subject_name, time_slot, date):
            ...
            flash(error_message, 'error')  # Use flash to store the error message
            return redirect(url_for('input'))  # Redirect to the input route
...
def attendance_summary():
    ...
    for subject in subjects:
        ...
        if time_slot_result:
            time_slot = time_slot_result[0]['time']
        else:
            time_slot = 'N/A'
        ...
        if summary:
            ...
            # No records for the subject on the given date
            subject_counts[subject] = {
                'present_count': 0,
                'absent_count': 0,
                'time_slot': time_slot,
